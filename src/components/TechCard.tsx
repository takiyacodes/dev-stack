...
const TechCard = ({ tech, onAddToStack, isSelected }: TechCardProps) => {
  return (
    <div className="bg-white/80 backdrop-blur-sm border border-slate-200/80 rounded-2xl p-6 flex flex-col justify-between h-full hover:border-pink-500/50 transition-all duration-300 shadow-sm">
      <div>
        <div className="flex items-center justify-between mb-4">
          <img src={tech.icon} alt={tech.name} className="w-12 h-12 object-contain" />
          <span className="bg-pink-500/10 text-pink-400 text-xs px-3 py-1 rounded-full border border-pink-500/20 font-medium">
            {tech.badge}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-slate-700 text-xs">• {tech.difficulty}</span>
        </div>
